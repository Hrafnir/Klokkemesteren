...
// ... (showScreen, initializeGame, setDefaultPracticeOptions uendret) ...
...
// ** setDigitalTime - Viser nå kun 12-timers format likt på begge **
function setDigitalTime(hour, minute) {
     // Kalkuler 12-timers representasjon (1-12) fra intern 0-23 time
     let displayHour12 = hour % 12;
     if (displayHour12 === 0) displayHour12 = 12; // Både 0 (midnatt) og 12 (middag) vises som 12
...
function displayTime(hour, minute){
    // Sikrer at interne verdier er gyldige 0-23 / 0-59
...
function getSmallestIntervalStep() {
    // Finner det *laveste* sifferet blant de valgte vanskelighetsgradene som er > 1
    const checkedDifficulties = Array.from(customPracticeSettings.querySelectorAll('input[id^="diff-"]:checked'))
                                    .map(cb => parseInt(cb.value, 10));
    if (checkedDifficulties.includes(5)) return 1;  // 'Alle minutter' overstyrer
    if (checkedDifficulties.includes(4)) return 5;  // '5-minutter'
    if (checkedDifficulties.includes(3)) return 15; // 'Kvart over/på'
    if (checkedDifficulties.includes(2)) return 30; // 'Halvtimer'
    if (checkedDifficulties.includes(1)) return 60; // 'Hele timer'
    return 1; // Fallback hvis ingenting er valgt (bør ikke skje pga sjekk i generateNewTask)
}
...
// ** Oppdatert drag() med riktig snapping **
function drag(x, y) {
    ...
    // Hent minste intervall basert på *valgte* vanskelighetsgrader
    const step = getSmallestIntervalStep();
    console.log("Smallest interval step:", step);

    if (draggedHand === 'minute') {
        // Kalkuler rå minuttverdi og snap den
        const rawMinuteFromAngle = (angle / 360) * 60;
        let snappedMinute = Math.round(rawMinuteFromAngle / step) * step;
        newMinute = (snappedMinute % 60 + 60) % 60; // Sikre 0-59
        // La timen være som den var, displayTime justerer visuelt
        newHour = currentHour;

    } else if (draggedHand === 'hour') {
        // Kalkuler time basert på vinkel
        ...
        // VIKTIG: Ikke endre minuttet basert på time-drag-vinkel!
        // La minuttet være som det var. Snapping skjer kun ved minutt-drag.
