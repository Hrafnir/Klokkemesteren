...
function showScreen(screenToShow) {
    ...
    if (screenToShow === 'setup') {
        ...
        if (gameModeActive) {
            ...
            backToSettingsButtonGame.classList.remove('hidden');
        } else {
            gameStatusBar.classList.add('hidden');
            changePracticeSettingsButton.classList.remove('hidden');
            backToSettingsButtonGame.classList.add('hidden');
        }
         gameOverMessage.classList.add('hidden');
         nextTaskButton.classList.add('hidden');
    } else if (screenToShow === 'gameOver') {
         gameWrapper.classList.remove('hidden'); // Må være synlig for å vise game over inni
...
function setDefaultPracticeOptions() {
    ...
    document.getElementById('mode-set-analog').checked = true; // Endret til true
    ...
    document.getElementById('diff-4').checked = false;
    document.getElementById('diff-5').checked = false;
}
...
// ** OPPDATERT setDigitalTime for korrekt AM(12t) / PM(24t) visning **
function setDigitalTime(hour, minute) { // Mottar intern time (0-23)
    const formattedMinute = formatTwoDigits(minute);

    // Formiddagsklokke (AM): Viser 12-timers format
    let displayHourAm = hour % 12;
    if (displayHourAm === 0) displayHourAm = 12; // 0 blir 12 AM
    digitalHourAm.textContent = formatTwoDigits(displayHourAm);
...
function displayTime(hour, minute){
    let cleanHour = Math.round(hour);
    let cleanMinute = Math.round(minute);
    cleanHour = (cleanHour % 24 + 24) % 24; // Sikrer 0-23
    cleanMinute = (cleanMinute % 60 + 60) % 60; // Sikrer 0-59

    currentHour = cleanHour;
    currentMinute = cleanMinute;
    console.log(`displayTime updated: currentHour=${currentHour}, currentMinute=${currentMinute}`);

    setAnalogTime(currentHour, currentMinute);
    setDigitalTime(currentHour, currentMinute); // Kaller oppdatert funksjon
}

function timeToText(hour, minute, difficultyLevel){
    const difficulty = parseInt(difficultyLevel, 10);
    let displayHour = hour % 12;
    if (displayHour === 0) displayHour = 12; // 0 og 12 vises som 12
...
function getRandomTime(difficulty) {
    ...
    // 50% sjanse for PM (timer 12-23), ellers AM (timer 0-11)
    if (Math.random() < 0.5) { // PM
        if (internalHour !== 12) internalHour += 12; // 1-11 PM blir 13-23
        // 12 PM forblir 12
    } else { // AM
        if (internalHour === 12) internalHour = 0; // 12 AM blir 0
        // 1-11 AM forblir 1-11
    }
...
function startTaskTimer() {
    if (!gameModeActive) return; stopTaskTimer();
    remainingTime = MAX_TIME; timerDisplay.textContent = remainingTime; timerDisplay.classList.remove('low-time');
    taskTimerInterval = setInterval(() => {
        remainingTime--; timerDisplay.textContent = remainingTime;
...
function handleTimeout() {
    ...
    stopTaskTimer(); taskActive = false; lives--; updateLivesDisplay();
    feedbackElement.textContent = "⏰ Tiden er ute!"; feedbackElement.className = 'feedback incorrect';
    analogClockElement.classList.remove('interactive-clock'); checkAnswerButton.classList.add('hidden');
    answerOptionsContainer.querySelectorAll('button').forEach(btn => btn.disabled = true);
    if (lives <= 0) { gameOver(); } else { nextTaskButton.classList.remove('hidden'); }
}
function generateNewTask() {
    if (isGameOver) return;
    console.log("generateNewTask: Starting..."); clearTimeout(autoNextTaskTimeout); stopTaskTimer();
    taskActive = false; console.log("generateNewTask: taskActive set to false");
    feedbackElement.textContent = ''; feedbackElement.className = 'feedback';
    ...
    if (checkedModes.length === 0 || checkedDifficulties.length === 0) {
        feedbackElement.textContent = "Velg minst én oppgavetype og én vanskelighetsgrad i innstillingene!";
        feedbackElement.className = 'feedback incorrect'; displayTime(12, 0);
        taskActive = false; showScreen('setup'); return;
    }
    ...
    if (currentTaskMode === 'set_analog') {
        ...
        displayTime(12, 0); currentHour = 12; currentMinute = 0; // Nullstill klokka for brukeren
    ...
    taskActive = true; console.log("generateNewTask: Task setup complete. taskActive set to true.");
...
function handleCheckAnswer() { // Sjekker visuell 12t-match
    console.log("handleCheckAnswer called!");
    console.log(`   State check: taskActive=${taskActive}, isGameOver=${isGameOver}`);
    if (!taskActive || isGameOver) { console.log("   handleCheckAnswer blocked by state."); return; }
    console.log(`   CHECKING: currentHour=${currentHour}, currentMinute=${currentMinute}`);
    console.log(`   CHECKING: targetHour=${targetHour}, targetMinute=${targetMinute}`);
    const currentHourMod12 = currentHour % 12; const targetHourMod12 = targetHour % 12;
    console.log(`   COMPARING: currentHourMod12=${currentHourMod12}, targetHourMod12=${targetHourMod12}`);
    console.log(`   COMPARING: currentMinute=${currentMinute}, targetMinute=${targetMinute}`);
    if (currentHourMod12 === targetHourMod12 && currentMinute === targetMinute) {
        console.log("   Check PASSED!"); feedbackElement.textContent = "Riktig stilt! 👍";
        handleCorrectAnswer();
    } else {
        console.log("   Check FAILED!"); const difficultyForText = currentDifficulty || '5';
        const userTimeString = timeToText(currentHour, currentMinute, difficultyForText);
        const correctTimeString = timeToText(targetHour, targetMinute, difficultyForText);
        handleIncorrectAnswer(`Du stilte ${userTimeString}. Riktig var ${correctTimeString}.`);
        setAnalogTime(targetHour, targetMinute); setDigitalTime(targetHour, targetMinute);
    }
}
...
function getSmallestIntervalStep() {
    const checkedDifficulties = Array.from(customPracticeSettings.querySelectorAll('input[id^="diff-"]:checked')).map(cb => parseInt(cb.value, 10));
    if (checkedDifficulties.includes(5)) return 1; if (checkedDifficulties.includes(4)) return 5;
    if (checkedDifficulties.includes(3)) return 15; if (checkedDifficulties.includes(2)) return 30;
    if (checkedDifficulties.includes(1)) return 60; return 1;
}
...
function drag(x, y) { // Korrekt logikk fra forrige runde
    if (!isDragging || !taskActive || isGameOver) return;
    const angle = getAngle(x, y); let newHour = currentHour; let newMinute = currentMinute;
    const step = getSmallestIntervalStep();

    if (draggedHand === 'minute') {
        const rawMinuteFromAngle = (angle / 360) * 60;
        let snappedMinute = Math.round(rawMinuteFromAngle / step) * step;
        newMinute = (snappedMinute % 60 + 60) % 60;
        newHour = currentHour; // Behold timen
    } else if (draggedHand === 'hour') {
        let preciseHour12 = (angle / 360) * 12;
        let roundedHour12 = Math.round(preciseHour12) % 12;
        let wasPM = currentHour >= 12;
        let displayHour12 = roundedHour12; if (displayHour12 === 0) displayHour12 = 12;
        if (wasPM) { newHour = displayHour12 === 12 ? 12 : displayHour12 + 12; }
        else { newHour = displayHour12 === 12 ? 0 : displayHour12; }
        newMinute = currentMinute; // La minutt være uendret
    }
    if (newHour !== currentHour || newMinute !== currentMinute) { displayTime(newHour, newMinute); }
}
function endDrag() {
    if (!isDragging) return; console.log("End dragging"); isDragging = false; draggedHand = null;
    if (taskActive && currentTaskMode === 'set_analog' && !isGameOver) { analogClockElement.style.cursor = 'grab'; }
    else { analogClockElement.style.cursor = 'default'; }
}
