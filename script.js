...
function setDefaultPracticeOptions() {
    ...
    document.getElementById('mode-set-analog').checked = false;
...
function generateNewTask() {
    ...
    if (checkedModes.length === 0 || checkedDifficulties.length === 0) {
        ...
        taskActive = false;
        showScreen('setup'); // Send tilbake til setup hvis ingen valg er gyldige
        return;
    }
...
function handleCheckAnswer() {
    ...
    // Sammenlign klokkeslettet på en 12-timers basis (ignorer AM/PM)
    const currentHourMod12 = currentHour % 12;
    const targetHourMod12 = targetHour % 12;

    // Sjekk om timen (0-11) og minuttet (0-59) stemmer overens
    if (currentHourMod12 === targetHourMod12 && currentMinute === targetMinute) {
        ...
        // Kall handleIncorrectAnswer med den tydelige feilmeldingen
        handleIncorrectAnswer(`Du stilte ${userTimeString}. Riktig var ${correctTimeString}.`);
        // Korriger klokka til riktig tid for å vise brukeren
